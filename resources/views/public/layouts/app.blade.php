...
<head>
    ...
    {{-- Vite (includes app.css with all public styles) --}}
...
<footer>
    <div class="footer-grid">
        <div>
            ...
            <p class="footer-desc">
                {{ (isset($company) && $company) ? $company->tagline : 'Holding Company Universitas Airlangga — membangun ekosistem bisnis berbasis riset dan inovasi.' }}
            </p>
            @if(isset($company) && $company)
    ...
    <div class="footer-bottom">
        <span>&copy; {{ date('Y') }} PT Dharma Putra Airlangga. Hak Cipta Dilindungi.</span>
...
{{-- SweetAlert Flash Messages — use data attributes set by PHP, read by JS --}}
@php
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    Swal.fire({
        icon: 'error',
        title: 'Oops!',
        text: {{ Js::from($flashError) }},
        confirmButtonColor: '#1a3a6e',
    });
    @elseif(count($flashErrors) > 0)
    Swal.fire({
        icon: 'warning',
        title: 'Periksa form Anda',
        html: {{ Js::from(implode('<br>', $flashErrors)) }},
        confirmButtonColor: '#1a3a6e',
    });
    @endif
});
