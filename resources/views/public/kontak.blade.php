    ...
    <div style="max-width:1100px;margin:0 auto;display:grid;grid-template-columns:1fr 1.4fr;gap:40px;align-items:start;">
        ...
        <div style="display:flex;flex-direction:column;gap:20px;">
            <div class="card-pub" style="padding:28px;">
                ...
                @if($company->alamat)
                @php
                    $contactMapsQuery = trim(($company->alamat ?? '') . ' ' . ($company->kota ?? '') . ' ' . ($company->kode_pos ?? ''));
                    ...
                    $contactMapEmbedUrl = 'https://www.google.com/maps?q=' . urlencode($contactMapsQuery) . '&z=15&output=embed';
                @endphp
                <div style="display:flex;gap:14px;margin-bottom:10px;align-items:flex-start;">
                    ...
                    <div style="flex:1;">
                        ...
                        <a href="{{ $contactMapsUrl }}" target="_blank" rel="noopener noreferrer" style="font-size:13.5px;color:var(--primary-light);line-height:1.6;text-decoration:none;display:inline-block;">
                            {{ $company->alamat }}{{ $company->kota ? ', '.$company->kota : '' }}{{ $company->kode_pos ? ' '.$company->kode_pos : '' }}
                        </a>
                    </div>
                </div>

                <div style="margin-top:12px;border-radius:12px;overflow:hidden;border:1px solid #dfeaf7;background:#f8fbff;">
                    <iframe
                ...
                <div style="display:flex;gap:14px;align-items:flex-start;">
                    ...
                    <div>
                        ...
                        <div style="font-size:13.5px;color:var(--text-muted);white-space:pre-line;">{{ $company->jam_operasional }}</div>
                    </div>
                </div>
                @endif
            </div>
            ...
            <div class="card-pub" style="padding:24px;">
                ...
                <div style="display:flex;flex-direction:column;gap:10px;">
                    @if($company->instagram)
                        @php
                            $instagramUrl = preg_match('/^https?:\/\//i', $company->instagram)
                                ? $company->instagram
                                : 'https://instagram.com/' . ltrim($company->instagram, '@');
                        @endphp
                        <a href="{{ $instagramUrl }}" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:10px;font-size:14px;text-decoration:none;color:var(--text);padding:8px;border-radius:8px;transition:background .2s;" onmouseover="this.style.background='#f0f4ff'" onmouseout="this.style.background='transparent'">
                            📷 <span>{{ $company->instagram }}</span>
                        </a>
                    @endif
                    @if($company->tiktok)
                        @php
                            $tiktokUrl = preg_match('/^https?:\/\//i', $company->tiktok)
                                ? $company->tiktok
                                : 'https://www.tiktok.com/@' . ltrim($company->tiktok, '@');
                        @endphp
                        <a href="{{ $tiktokUrl }}" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:10px;font-size:14px;text-decoration:none;color:var(--text);padding:8px;border-radius:8px;transition:background .2s;" onmouseover="this.style.background='#f0f4ff'" onmouseout="this.style.background='transparent'">
...
{{-- LocalBusiness JSON-LD (inline to avoid @if/@endif inside @push parse error) --}}
