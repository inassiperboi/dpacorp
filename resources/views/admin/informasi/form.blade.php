...
@section('page-title', isset($informasi) ? 'Edit Informasi' : 'Tambah Informasi')
{{-- @section('breadcrumb') / <a href="{{ route('admin.news-information.index') }}">Berita & Informasi</a> / <a href="{{ route('admin.informasi.index') }}">Informasi</a> / {{ isset($informasi) ? 'Edit' : 'Tambah' }}@endsection --}}

@php($currentInformasi = $informasi ?? null)

@section('content')
<form action="{{ isset($currentInformasi) ? route('admin.informasi.update', $currentInformasi) : route('admin.informasi.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    @if(isset($currentInformasi)) @method('PUT') @endif

<div style="display:grid;grid-template-columns:1.4fr 1fr;gap:20px;align-items:start;">
    <div class="card">
        <div class="card-header"><h3>Konten Informasi</h3><a href="{{ route('admin.informasi.index') }}" class="btn btn-secondary btn-sm">← Kembali</a></div>
        <div class="card-body">
            <div class="form-group">
                <label class="form-label">Judul <span class="required">*</span></label>
                <input type="text" name="title" id="title" class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" value="{{ old('title', $currentInformasi->title ?? '') }}" required>
                @error('title')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="form-group">
                <label class="form-label">Slug <span class="required">*</span></label>
                <input type="text" name="slug" id="slug" class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" value="{{ old('slug', $currentInformasi->slug ?? '') }}" required>
                @error('slug')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="form-group">
                <label class="form-label">Ringkasan <span class="required">*</span></label>
                <textarea name="summary" class="form-control {{ $errors->has('summary') ? 'is-invalid' : '' }}" rows="4" required>{{ old('summary', $currentInformasi->summary ?? '') }}</textarea>
                @error('summary')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="form-group">
                <label class="form-label">Isi Konten <span class="required">*</span></label>
                <textarea name="content" class="form-control {{ $errors->has('content') ? 'is-invalid' : '' }}" rows="12" required>{{ old('content', $currentInformasi->content ?? '') }}</textarea>
                @error('content')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div class="form-group">
                <label class="form-label">Tags <span class="required">*</span></label>
                <div class="tag-chip-field {{ $errors->has('tags') ? 'is-invalid' : '' }}" id="informasi-tags-field">
                    <div class="tag-chip-list" id="informasi-tags-list"></div>
                    <input type="text" class="tag-chip-input" id="informasi-tags-input" placeholder="Ketik tag lalu tekan Enter">
                    <textarea name="tags" id="informasi-tags-value" hidden>{{ old('tags', $currentInformasi->tags ?? '') }}</textarea>
                </div>
    ...
    <div style="display:flex;flex-direction:column;gap:20px;">
        <div class="card">
            ...
            <div class="card-body">
                @if($currentInformasi && $currentInformasi->image)
                    <img src="{{ asset('storage/'.$currentInformasi->image) }}" alt="{{ $currentInformasi->title }}" style="width:100%;max-height:220px;object-fit:cover;border-radius:10px;margin-bottom:12px;">
                @endif
                <div class="form-group">
                    <label class="form-label">Upload Gambar</label>
                    <input type="file" name="image" class="form-control {{ $errors->has('image') ? 'is-invalid' : '' }}" accept="image/jpeg,image/png,image/jpg">
                    <div class="form-hint">Opsional untuk informasi.</div>
                    @error('image')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Gambar Tambahan</label>
                    <div class="dropzone-file {{ $errors->has('image_2') ? 'is-invalid' : '' }}" id="informasi-image-dropzone">
                        <input type="file" name="image_2[]" id="informasi-image-input" accept="image/jpeg,image/png,image/jpg" multiple hidden>
                        ...
                        <div class="dropzone-files" id="informasi-image-files"></div>
                    </div>
                    ...
                    @error('image_2.*')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    @if($currentInformasi && $currentInformasi->extraImages())
                        <div style="margin-top:12px;display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:10px;">
                            @foreach($currentInformasi->extraImages() as $image)
                                <img src="{{ asset('storage/'.$image) }}" alt="{{ $currentInformasi->title }}" style="width:100%;height:90px;object-fit:cover;border-radius:10px;border:1px solid var(--border);">
                            @endforeach
                        </div>
                    @endif
                </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" name="country" class="form-control {{ $errors->has('country') ? 'is-invalid' : '' }}" value="{{ old('country', $currentInformasi->country ?? '') }}" required>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const tagsField = document.getElementById('informasi-tags-field');
    const tagsList = document.getElementById('informasi-tags-list');
    const tagsInput = document.getElementById('informasi-tags-input');
    const tagsValue = document.getElementById('informasi-tags-value');
    const imageDropzone = document.getElementById('informasi-image-dropzone');
    const imageInput = document.getElementById('informasi-image-input');
    const imageFiles = document.getElementById('informasi-image-files');
